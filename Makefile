...
#---------------------------------------------------------------------------------------------------------------------
TARGET      	:=  luar-assault-v0.5.1
BUILD       	:=  build
LIBBUTANO   	:=  ../butano/butano
PYTHON      	:=  python
SOURCES     	:=  src src/fr_lib src/enemies src/scenes src/cutscene ../butano/common/src
INCLUDES    	:=  include include/fr_lib include/game_scene_defs include/enemies include/scenes include/cutscene ../butano/common/include $(BUILD)
DATA        	:=
...
LOCALFONTS		:=  fonts
FONTS     	 	:=  $(GBAFONTS)/SourceHanSans/JP $(GBAFONTS)/SourceHanMono/JP $(LOCALFONTS)/VonwaonBitmap $(LOCALFONTS)/k8x8 $(LOCALFONTS)/VonwaonBitmapBlack $(LOCALFONTS)/Tanklager
TEXTS           :=  src/main.cpp 
