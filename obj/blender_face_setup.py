bl_info = {
    "name": "Face Attribute Setter",
    "author": "GitHub Copilot",
    "version": (1, 1),
    "blender": (4, 5, 0),
    "location": "View3D > Sidebar > Face Attribute Setter",
    "description": "Set custom integer attribute for selected faces (safe for Blender 4.5+)",
    "category": "Mesh",
}
...
class MESH_PT_face_attribute_setter(bpy.types.Panel):
    bl_label = "Face Attribute Setter"
    bl_idname = "MESH_PT_face_attribute_setter"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = "Face Attribute Setter"
    ...
    def draw(self, context):
        ...
        if obj is None or obj.type != 'MESH' or bpy.context.object.mode != 'EDIT':
            layout.label(text="Switch to Edit Mode and select a mesh.", icon='ERROR')
            return
        bm = bmesh.from_edit_mesh(obj.data)
        selected_faces = [f for f in bm.faces if f.select]
        if not selected_faces:
            layout.label(text="Select at least one face.", icon='ERROR')
            return
        ...
        layout.prop(context.scene, "fa_attr_value")
        layout.operator("mesh.set_face_attribute", text="Set Engine Brightness")
...
class MESH_OT_set_face_attribute(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Ensure bmesh integer layer exists for faces
