...
class model_3d : public bn::intrusive_list_node_type
{
  ...
  public:
    constexpr explicit model_3d(const model_3d_item &item) : _item(item)
    {
    }
    ...
    [[nodiscard]] constexpr point_3d rotate(const vertex_3d &vertex) const
    {
        ...
        bn::fixed rx = (_xx + vy).safe_multiplication(_xy + vx) +
                       vz.unsafe_multiplication(_xz) - _xx_xy - vxy;
        bn::fixed ry = (_yx + vy).safe_multiplication(_yy + vx) +
                       vz.unsafe_multiplication(_yz) - _yx_yy - vxy;
        bn::fixed rz = (_zx + vy).safe_multiplication(_zy + vx) +
                       vz.unsafe_multiplication(_zz) - _zx_zy - vxy;
