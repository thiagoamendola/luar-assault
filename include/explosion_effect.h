...
class explosion_effect
{
  ...
  private:
    ...
    fr::sprite_3d_item _sprite_3d_item;
    fr::sprite_3d* _sprite = nullptr;

    int _current_frame;
    bn::fixed _initial_scale;
    bn::fixed _theta;

    static constexpr int ANIMATION_SPEED = 4; // frames per animation frame
    static constexpr int TOTAL_FRAMES = 10;
    static constexpr bn::fixed INITIAL_SCALE = 1.5;
    static constexpr bn::fixed FINAL_SCALE = 2.5;
    static constexpr bn::fixed INITIAL_THETA = 40000;
};
